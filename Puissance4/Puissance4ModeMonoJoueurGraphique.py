...
from JouerTourCompletIAGraphique import *
from time import *
################################## Programme ###############################
...
def ClicSurDamierGraphique(event):
   ...
   if not partie_finie and not match_nul and colonne_choisie!=float('inf') :#si la partie n'est pas finit, qu'il n'y a pas match nul et que le joueur a selectionné une colonne du damier
        ...
        x0,y0,dernierCoup,ymaxGraphique,nbPionsJoueur2 =JouerTourCompletIAGraphiquet(nb_joueur2,damier,nbPionsJoueur2)# x0,y0 corespondent au coordonnees de la case la plus haute de la colonne selectionnée dans le graphique par le joueur 2 ; dernierCoup à [colonne_choisie, ligne du dernier coup ] ; ymaxGraphique au y dans la fenetre de jeu sur lequel le pion rouge tombe quand un coup est joué
        Image=canvas.create_image(x0,y0, image=imagePionRouge) #affiche un Pion jaune en x0,y0
        AnimationDeplacementPion(canvas,Image,ymaxGraphique)
...
imageDamier=PhotoImage(file="damierVide.png").zoom(5).subsample(5)
